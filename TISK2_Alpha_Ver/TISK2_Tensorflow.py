...
class Weight_Generator:
    ...
    def Trained_Weight_Initialize(self):
        self.weightMatrix_SW = tf.Variable(tf.zeros((len(self.single_Phone_List), len(self.word_List)), dtype=tf.float32));
        self.weightMatrix_DW = tf.Variable(tf.zeros((len(self.diphone_List), len(self.word_List)), dtype=tf.float32));
        self.biasMatrix_W = tf.Variable(tf.zeros((1, len(self.word_List)), dtype=tf.float32));

        self.tf_Session.run(tf.variables_initializer([
            self.weightMatrix_SW,
            self.weightMatrix_DW,
            self.biasMatrix_W,
        ]));

    def Tensor_Generate(self):
        self.single_Phone_Pattern_Placeholder = tf.placeholder("float32");
        self.diphone_Pattern_Placeholder = tf.placeholder("float32");
        self.word_Pattern_Placeholder = tf.placeholder("float32");
        self.learning_Rate_Placeholder = tf.placeholder("float32");

        #Phone to Word
        word_Activation = tf.clip_by_value(tf.matmul(self.single_Phone_Pattern_Placeholder, self.weightMatrix_SW) + tf.matmul(self.diphone_Pattern_Placeholder, self.weightMatrix_DW), 0, 1);
        word_Error = self.word_Pattern_Placeholder - word_Activation
        word_MSE = tf.reduce_mean(tf.sqrt(tf.reduce_mean((self.word_Pattern_Placeholder - word_Activation) ** 2, axis=1)))

        assign_WeightMatrix_SW = tf.assign_add(self.weightMatrix_SW, tf.clip_by_value(self.learning_Rate_Placeholder * tf.matmul(tf.transpose(self.single_Phone_Pattern_Placeholder), word_Error), 0, np.inf));
        assign_WeightMatrix_DW = tf.assign_add(self.weightMatrix_DW, tf.clip_by_value(self.learning_Rate_Placeholder * tf.matmul(tf.transpose(self.diphone_Pattern_Placeholder), word_Error), 0, np.inf));

        self.phone_To_Word_Tensor_List = [];
        self.phone_To_Word_Tensor_List.extend([word_MSE, word_Activation]);
        self.phone_To_Word_Tensor_List.extend([assign_WeightMatrix_SW, assign_WeightMatrix_DW]);

    def Phone_to_Word_Train(self, max_Epoch = 500, learning_Rate = 0.01):
        for epoch in range(max_Epoch):
            start_Time = time.time();
            result_List = self.tf_Session.run(
                self.phone_To_Word_Tensor_List,
                feed_dict = {
                    self.learning_Rate_Placeholder: learning_Rate,
                    self.single_Phone_Pattern_Placeholder: self.single_Phone_Pattern,
                    self.diphone_Pattern_Placeholder: self.diphone_Pattern,
                    self.word_Pattern_Placeholder: self.word_Pattern,
                    }
                );
            word_Activation = result_List[1];
    ...
    def Extract(self, file_Name = "Basic_Weight.pickle"):
        ...
        trained_Weights_List = self.tf_Session.run([
            self.weightMatrix_SW,
            self.weightMatrix_DW,
            self.biasMatrix_W,
            ]);
        ...
        save_Dict[("Weight", "Single_Phone_to_Word")] = trained_Weights_List[0];
        save_Dict[("Weight", "Diphone_to_Word")] = trained_Weights_List[1];
        save_Dict[("Weight", "Word_Bias")] = trained_Weights_List[2];
...
class TISK2_Model:
    def __init__(self, file_Name = "Basic_Weight.pickle"):
        ...
        self.weightMatrix_Single_Phone_to_Word = tf.Variable(load_Dict[("Weight", "Single_Phone_to_Word")]);
        self.weightMatrix_Diphone_to_Word = tf.Variable(load_Dict[("Weight", "Diphone_to_Word")]);
        self.biasMatrix_Word = tf.Variable(load_Dict[("Weight", "Word_Bias")]);
        self.weightMatrix_Word_to_Word = tf.Variable(load_Dict[("Weight", "Word_to_Word")]);

        self.weightMatrix_Word_to_Single_Phone = tf.Variable(load_Dict[("Feedback", "Word_to_SPhone")]);
        self.weightMatrix_Word_to_Diphone = tf.Variable(load_Dict[("Feedback", "Word_to_Diphone")]);

        self.tf_Session.run(tf.variables_initializer([
            self.weightMatrix_Single_Phone_to_Word,
            self.weightMatrix_Single_Phone_to_Word,
            self.weightMatrix_Diphone_to_Word,
            self.biasMatrix_Word,
            self.weightMatrix_Word_to_Word,

            self.weightMatrix_Word_to_Single_Phone,
            self.weightMatrix_Word_to_Diphone,
        ]));
    ...
    def Run(self, pronunciation, activation_Ratio_Dict = {}):
        ...
        input_Location = np.zeros(shape = (self.parameter_Dict["time_Slots"], 1, self.phoneme_Amount * self.parameter_Dict["time_Slots"]), dtype=np.float32);
        gate_Phoneme_to_Diphone = np.zeros(shape=(self.parameter_Dict["time_Slots"], 1, self.phoneme_Amount*self.parameter_Dict["time_Slots"], self.diphone_Amount), dtype=np.float32) + 1; #Initially all gates have state 1

        for slot_Index in range(self.parameter_Dict["time_Slots"]):
            input_Location[slot_Index, :, slot_Index*self.phoneme_Amount:(slot_Index+1)*self.phoneme_Amount] = 1;

        for slot_Index in range(self.parameter_Dict["time_Slots"] - 1):
            gate_Phoneme_to_Diphone[slot_Index + 1] = gate_Phoneme_to_Diphone[slot_Index];
                ...
                for diphone_Index in range(self.diphone_Amount):
                    if pronunciation[slot_Index] == self.diphone_List[diphone_Index][0] and pronunciation[slot_Index] != self.diphone_List[diphone_Index][1]: #Forward phone is same to inserted, and bacward phone is different
                        for slot_Index_for_Gate in range(slot_Index + 1, self.parameter_Dict["time_Slots"]):   #This mean closing process only affect the slots which are after current slot.
                            gate_Phoneme_to_Diphone[slot_Index + 1, 0, slot_Index_for_Gate * self.phoneme_Amount + self.phoneme_List.index(pronunciation[slot_Index]),diphone_Index] = 0;    #Assign 0

        phoneme_Activation_Cycle, diphone_Activation_Cycle, single_Phone_Activation_Cycle, word_Activation_Cycle = self.tf_Session.run(
            self.run_Tensor_List,
            feed_dict={
                self.using_Pattern_Placeholder: using_Pattern,
                self.input_Location_Placeholder: input_Location,
                self.gate_Phoneme_to_Diphone_Placeholder: gate_Phoneme_to_Diphone,
            })
    ...
    def Multi_Run(self, pronunciation_List):
        ...
        input_Location = np.zeros(shape = (self.parameter_Dict["time_Slots"], len(pronunciation_List), self.phoneme_Amount * self.parameter_Dict["time_Slots"]), dtype=np.float32);
        gate_Phoneme_to_Diphone = np.zeros(shape=(self.parameter_Dict["time_Slots"], len(pronunciation_List), self.phoneme_Amount*self.parameter_Dict["time_Slots"], self.diphone_Amount), dtype=np.float32) + 1; #Initially all gates have state 1
            ...
            for pronunciation_Index in range(len(pronunciation_List)):
                ...
                if slot_Index < len(pronunciation): #If slot_Index is same or bigger than length of pronunciation, there is no input
                    for diphone_Index in range(self.diphone_Amount):
                        if pronunciation[slot_Index] == self.diphone_List[diphone_Index][0] and pronunciation[slot_Index] != self.diphone_List[diphone_Index][1]: #Forward phone is same to inserted, and bacward phone is different
                            for slot_Index_for_Gate in range(slot_Index + 1, self.parameter_Dict["time_Slots"]):   #This mean closing process only affect the slots which are after current slot.
                                gate_Phoneme_to_Diphone[slot_Index + 1, pronunciation_Index, slot_Index_for_Gate * self.phoneme_Amount + self.phoneme_List.index(pronunciation[slot_Index]),diphone_Index] = 0;    #Assign 0
    ...
    def Tensor_Generate(self):
        self.using_Pattern_Placeholder = tf.placeholder(tf.float32);
        self.input_Location_Placeholder = tf.placeholder(tf.float32);
        self.gate_Phoneme_to_Diphone_Placeholder = tf.placeholder(tf.float32);
        ...
        batch_Size = tf.shape(self.using_Pattern_Placeholder)[0];
        ##Layer Initialize
        phoneme_Layer_Activation = tf.zeros(shape = (batch_Size, self.phoneme_Amount * self.parameter_Dict["time_Slots"]));
        diphone_Layer_Activation = tf.zeros(shape = (batch_Size, self.diphone_Amount));
        single_Phone_Layer_Activation = tf.zeros(shape = (batch_Size, self.phoneme_Amount));
        word_Layer_Activation = tf.zeros(shape = (batch_Size, self.word_Amount));

        for slot_Index in range(self.parameter_Dict["time_Slots"]):
            #Time control (The current phoneme location of pronunication)
            located_Input = self.using_Pattern_Placeholder * self.input_Location_Placeholder[slot_Index];
            for step_Index in range(self.parameter_Dict["iStep"]):
                phoneme_Layer_Stroage = located_Input * self.parameter_Dict[("Weight", "Input_to_Phoneme")];
                gated_WeightMatrix_Phoneme_to_Diphone = self.gate_Phoneme_to_Diphone_Placeholder[slot_Index] * self.weightMatrix_Phoneme_to_Diphone;
                diphone_Layer_Stroage = tf.reshape(
                    tf.matmul(tf.reshape(phoneme_Layer_Activation, [tf.shape(phoneme_Layer_Activation)[0], 1, tf.shape(phoneme_Layer_Activation)[1]]), gated_WeightMatrix_Phoneme_to_Diphone),
                    [tf.shape(gated_WeightMatrix_Phoneme_to_Diphone)[0], tf.shape(gated_WeightMatrix_Phoneme_to_Diphone)[2]]
                    );  #Because weight is 3D.
                diphone_Layer_Stroage = tf.sign((tf.sign(diphone_Layer_Stroage - self.parameter_Dict["nPhone_Threshold"]) + 1) / 2) / 10 + tf.matmul(word_Layer_Activation, self.weightMatrix_Word_to_Diphone);  #Binary + Feedback
                single_Phone_Layer_Stroage = tf.matmul(phoneme_Layer_Activation, self.weightMatrix_Phoneme_to_Single_Phone);
                single_Phone_Layer_Stroage = tf.sign((tf.sign(single_Phone_Layer_Stroage - self.parameter_Dict["nPhone_Threshold"]) + 1) / 2) / 10 + tf.matmul(word_Layer_Activation, self.weightMatrix_Word_to_Single_Phone);  #Binary + Feedback
                word_Layer_Stroage = tf.matmul(diphone_Layer_Activation, self.weightMatrix_Diphone_to_Word) + tf.matmul(single_Phone_Layer_Activation, self.weightMatrix_Single_Phone_to_Word) + tf.matmul(word_Layer_Activation, self.weightMatrix_Word_to_Word);

                phoneme_Layer_Activation = tf.clip_by_value(phoneme_Layer_Activation * (1 - self.parameter_Dict[("Decay", "Phoneme")]) - tf.abs(phoneme_Layer_Stroage) * phoneme_Layer_Activation + tf.clip_by_value(phoneme_Layer_Stroage, clip_value_min=0, clip_value_max=np.inf), clip_value_min=0, clip_value_max=1);
                diphone_Layer_Activation = tf.clip_by_value(diphone_Layer_Activation * (1 - self.parameter_Dict[("Decay", "Diphone")]) - tf.abs(diphone_Layer_Stroage) * diphone_Layer_Activation + tf.clip_by_value(diphone_Layer_Stroage, clip_value_min=0, clip_value_max=np.inf), clip_value_min=0, clip_value_max=1);
                single_Phone_Layer_Activation = tf.clip_by_value(single_Phone_Layer_Activation * (1 - self.parameter_Dict[("Decay", "SPhone")]) - tf.abs(single_Phone_Layer_Stroage) * single_Phone_Layer_Activation + tf.clip_by_value(single_Phone_Layer_Stroage, clip_value_min=0, clip_value_max=np.inf), clip_value_min=0, clip_value_max=1);
                word_Layer_Activation = tf.clip_by_value(word_Layer_Activation * (1 - self.parameter_Dict[("Decay", "Word")]) - tf.abs(word_Layer_Stroage) * word_Layer_Activation + tf.clip_by_value(word_Layer_Stroage, clip_value_min=0, clip_value_max=np.inf), clip_value_min=0, clip_value_max=1);
        ...
        phoneme_Activation_Cycle = tf.transpose(tf.stack(phoneme_Activation_Cycle_List), [1, 0, 2]);
        diphone_Activation_Cycle = tf.transpose(tf.stack(diphone_Activation_Cycle_List), [1, 0, 2]);
        single_Phone_Activation_Cycle = tf.transpose(tf.stack(single_Phone_Activation_Cycle_List), [1, 0, 2]);
        word_Activation_Cycle = tf.transpose(tf.stack(word_Activation_Cycle_List), [1, 0, 2]);

        self.run_Tensor_List = [phoneme_Activation_Cycle, diphone_Activation_Cycle, single_Phone_Activation_Cycle, word_Activation_Cycle];
    ...
    def Run_List(self, pronunciation_List, absolute_Acc_Criteria=0.75, relative_Acc_Criteria=0.05, time_Acc_Criteria=10, output_File_Name=None, raw_Data=False, categorize=False, reaction_Time=False, batch_Size=100):
        ...
        for batch_Index in range(0, len(pronunciation_List), batch_Size):
            print("Run_List Batch:", batch_Index, "to", min(batch_Index + batch_Size - 1, len(pronunciation_List)), "/", len(pronunciation_List));
            start_Time = time.time();
            ...
            for pronunciation_Index in range(min(len(pronunciation_List) - batch_Index, batch_Size)):
                ...
                phoneme_Activation_Array_List.append(phoneme_Activation_Array[pronunciation_Index].astype("float16"));
                diphone_Activation_Array_List.append(diphone_Activation_Array[pronunciation_Index].astype("float16"));
                single_Phone_Activation_Array_List.append(single_Phone_Activation_Array[pronunciation_Index].astype("float16"));
                word_Activation_Array_List.append(word_Activation_Array[pronunciation_Index].astype("float16"));
    ...
    def Average_Activation_by_Category_Graph(self, pronunciation_List, file_Save = False, output_File_Name = "Average_Activation_by_Category_Graph.png", batch_Size=100):
        ...
        for batch_Index in range(0, len(pronunciation_List), batch_Size):
            print("Average_Activation_by_Category_Graph Batch:", batch_Index, "to", min(batch_Index + batch_Size - 1, len(pronunciation_List)), "/", len(pronunciation_List));
            ...
            for pronunciation_Index in range(min(len(pronunciation_List) - batch_Index, batch_Size)):
                ...
                if len(target_Index_List) > 0:
                    target_Activation_List.append(np.mean(word_Activation_Array[:, target_Index_List], axis=1));
                if len(cohort_Index_List) > 0:
                    cohort_Activation_List.append(np.mean(word_Activation_Array[:, cohort_Index_List], axis=1));
                if len(rhyme_Index_List) > 0:
                    rhyme_Activation_List.append(np.mean(word_Activation_Array[:, rhyme_Index_List], axis=1));
                if len(embedding_Index_List) > 0:
                    embedding_Activation_List.append(np.mean(word_Activation_Array[:, embedding_Index_List], axis=1));
                if len(other_Index_List) > 0:
                    other_Activation_List.append(np.mean(word_Activation_Array[:, other_Index_List], axis=1));
        ...
        if len(target_Activation_List) > 0:
            ...
            display_Color_List.append("b");
            display_Marker_List.append(marker_list[1]);
        ...
        if len(cohort_Activation_List) > 0:
            ...
            display_Color_List.append("g");
            display_Marker_List.append(marker_list[2]);
        ...
        if len(rhyme_Activation_List) > 0:
            ...
            display_Color_List.append("r");
            display_Marker_List.append(marker_list[3]);
        ...
        if len(embedding_Activation_List) > 0:
            ...
            display_Color_List.append("c");
            display_Marker_List.append(marker_list[4]);
        ...
        if len(other_Activation_List) > 0:
            ...
            display_Color_List.append("m");
            display_Marker_List.append(marker_list[5]);

        fig = plt.figure(figsize=(8, 8));
        for y_arr, label, color, marker in zip(display_Data_List, display_Category_List, display_Color_List, display_Marker_List):
            plt.plot(list(range(self.parameter_Dict["time_Slots"] * self.parameter_Dict["iStep"])), y_arr, label=label, color=color, marker=marker);
...
if __name__ == "__main__":
    ...
    # print(new_TISK2_Model.Run_List(new_TISK2_Model.word_List, absolute_Acc_Criteria=0.85, relative_Acc_Criteria=0.05, time_Acc_Criteria=10,));
    ...
    # print(new_TISK2_Model.Run_List(new_TISK2_Model.word_List, absolute_Acc_Criteria=0.45, relative_Acc_Criteria=0.05, time_Acc_Criteria=10,));

    # 14898 words
    #
    # new_Weight_Generator = Weight_Generator(lexicon_File="14898_Lexicon.txt");
    # new_Weight_Generator.Weight_Parameter_Assign(word_to_Word_Weight = -0.0001)
    # new_Weight_Generator.Wired_Weights_Generate();
    # new_Weight_Generator.Phone_to_Word_Train(max_Epoch = 500);
    # new_Weight_Generator.Extract(file_Name="14898_Weight.pickle");
    #
    new_TISK2_Model = TISK2_Model(file_Name="14898_Weight.pickle");
    # print(new_TISK2_Model.Run_List(new_TISK2_Model.word_List, absolute_Acc_Criteria=0.8, relative_Acc_Criteria=0.03, time_Acc_Criteria=10, batch_Size = 100));
    new_TISK2_Model.Average_Activation_by_Category_Graph(new_TISK2_Model.word_List, file_Save = True, output_File_Name = "14898_AVG_Category.png", batch_Size = 100);


    # new_TISK2_Model.Display_Graph('ar', display_Word_List = ['ar', 'art', 'art^st'], file_Save=True)
    # new_TISK2_Model.Display_Graph('art', display_Word_List = ['ar', 'art', 'art^st'], file_Save=True)
    ...
    for length in range(1, 10):
        test_List = [x for x in new_TISK2_Model.word_List if len(x) == length];
        new_TISK2_Model.Average_Activation_by_Category_Graph(test_List, file_Save = True, output_File_Name = "Length_" + str(length) + "_14898_Category.png")
        plt.close();
